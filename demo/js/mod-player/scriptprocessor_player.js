...
// ******* (music) file input related
		...
		*/
		...
		/**
		* EDIT: Loads from a typed array - useful if dealing with an existing ArrayBuffer
		*/
		loadMusicFromTypedArray: function (name, typedArray, options, onCompletion, onFail, onProgress) {
			this.initByUserGesture();	// cannot be done from the callbacks below.. see iOS shit

			var filename= name;	// format detection may depend on prefixes and postfixes..
			...
			var pfn= this._backendAdapter.getPathAndFilename(filename);
			var data= new Uint8Array(typedArray);
			var fileHandle= this._backendAdapter.registerFileData(pfn, data);
			if (typeof fileHandle === 'undefined' ) {
				onFail();
				return;
			...
			this.prepareTrackForPlayback(fullFilename, typedArray, options);
			onCompletion(filename);
		},
